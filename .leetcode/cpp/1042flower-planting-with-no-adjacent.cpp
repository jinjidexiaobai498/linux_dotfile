...
class Solution {
    std::array<int, 5> colors = { 0, 1, 2, 4, 8 };

public:
    auto gardenNoAdj(int n, vector<vector<int>>& paths) -> vector<int>
    {
        vector<int> res(n, 0);
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= 4; j++) {
                if ((status[i] & colors[j]) == 0) {
                    res[i - 1] = j;
                    break;
                }
            }
            for (auto& point : map[i]) {
                if (point > i) {
                    status[point] |= colors[res[i - 1]];
                }
            }
        }
