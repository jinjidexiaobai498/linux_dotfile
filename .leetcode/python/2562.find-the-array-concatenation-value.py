...
# from python3problem2562 import *
# from typing import *
...
class Solution:
    ...
    def concTwoInt(a: int, b: int) -> int:
        tmp = b
        while tmp > 0:
            a *= Solution.BASE
            tmp //= Solution.BASE
    ...
    def findTheArrayConcVal(self, nums: List[int]) -> int:
        ...
        i = 0
        while i < rounds:
            sumConcVal += Solution.concTwoInt(nums[i], nums[-i - 1])
            i += 1
        if nums and len(nums) % 2 != 0:
            sumConcVal += nums[rounds]
        return sumConcVal
